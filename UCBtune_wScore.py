...
class UCBtune_wScore(MAB):
	...
	def select_items(self, required_num, item_score):
		...
		for i in range(self.K):
			...
			var = self.S[i]/self.N[i] - (self.S[i]/self.N[i])**0.5 + (2*np.log(self.turn)/self.N[i])**0.5
			items.append(((self.S[i]/self.N[i] + min(1/4, var)*(np.log(self.turn)/self.N[i])) * item_score[i], i))
